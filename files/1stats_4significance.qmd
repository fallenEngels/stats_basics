...
ggplot(left_1000, aes(x = perc)) + geom_density(bins = 30,
  ...
  labs(x = "Prevalence of left-handedness", y = "count") + theme_minimal()
...
ggplot(left_1000, aes(x = perc)) +
  ...
  geom_vline(xintercept = sample$mean, colour = "red") + 
  scale_x_continuous(limits = c(0.07, 0.16)) + 
  labs(x = "Est. prevalence of left-handedness", y = "count") + theme_minimal()
...
ggplot(left_1000, aes(x = perc)) +
  ...
  labs(x = "Value distribution with 95% intervals", y = "count") + theme_minimal()
...
This leads to an improvement in confidence intervals: 95% confidence and an equal split leaves 2.5% of samples below and 2.5% of cases above our calculated value. If we only focus on one side (i.e. we ask "is something significantly smaller than our result?"), we can shift the entire 5% "rest" over to that side. This is what's shown here. Because we only focus on if something is significantly smaller, the right-side line is now non-significant, while the left-side line lies outside the gray zone - even though the right-side line is farther from our sample's mean! Because of the way they're constructed, these types of tests are also called **two-tailed** or **one-tailed significance tests** (with two tails if measuring in both directions).
...
And also, don't blindly trust stars (this goes for real life and statistics!) - Just because one gets a star and the other doesn't, that does not mean that an effect with a 90% confidence interval is so much better than an effect with a 89.9% confidence interval. Use your head and think about your results!
...
**Source data for this page** can be found <a href="https://github.com/fallenEngels/stats-basics/blob/main/files/1stats_4significance.qmd">here</a>.
