...
sample_left = function(rep_n, sample_n){
  df <- data.frame(sample = seq(1, sample), perc = NA)
  for(i in 1:sample){
...
ggplot(left_10, aes(x = perc)) + geom_histogram(bins = 5,
  ...
  theme_minimal() + labs(x = "Prevalence of left-handedness", y = "count")
...
lastmodified = datetime.fromtimestamp(os.stat('1stats_2sampling.qmd').st_mtime)
...
**Source data for this page** can be found <a href="https://github.com/fallenEngels/stats-basics/blob/main/files/1stats_2sampling.qmd">here</a>.
