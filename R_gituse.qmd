...
Now that you've substantially improved someone else's code (or toyed around in a separate branch and want to update the main branch), another step is necessary: merging the changes.
If you're the owner of both the original and the new and improved branch, this is not an issue and can be done with relative ease. Out of convenience, I will demonstrate this using GitHub, but as before, you can also use [Git via terminal](https://happygitwithr.com/git-branches).
